...
-- ---------------------------------------------------
-- Client file.  Visit FiveM forums for
...
function SetLxSirenStateForVeh(veh, newstate)
	if DoesEntityExist(veh) and not IsEntityDead(veh) then
		if newstate ~= state_lxsiren[veh] then
			...
			if newstate == 1 then
				if GetEntityModel(veh) == 1938952078 then
	...
	if DoesEntityExist(veh) and not IsEntityDead(veh) then
		if toggle == true then
			if snd_pwrcall[veh] == nil then
				...
				if model == 1938952078 or model == 1171614426 or model == 469291905 or model == GetHashKey("AMBULANCE2") or model == GetHashKey("CAP89") or model == GetHashKey("CAP892") or model == GetHashKey("CVPIFD") or model == GetHashKey("EVAN") then
			...
			if IsPedInAnyVehicle(playerped, false) then	
				...
				if GetPedInVehicleSeat(veh, -1) == playerped then
					...
					if GetVehicleClass(veh) == 18 then
						...
						if GetEntityModel(veh) == 1938952078 and state_lxsiren[veh] == 1 then
							TogMuteDfltSrnForVeh(veh, false)
							dsrn_mute = false
						else
							TogMuteDfltSrnForVeh(veh, true)
					...
					if GetVehicleClass(veh) ~= 14 and GetVehicleClass(veh) ~= 15 and GetVehicleClass(veh) ~= 16 and GetVehicleClass(veh) ~= 21 then
						...
						if count_bcast_timer > delay_bcast_timer then
							count_bcast_timer = 0
							--- IS EMERG VEHICLE ---
							local source12 = PlayerId()
							if GetVehicleClass(veh) == 18 then
								TriggerServerEvent("lvc_TogDfltSrnMuted_s", source12,dsrn_mute)
								TriggerServerEvent("lvc_SetLxSirenState_s", source12,state_lxsiren[veh])
								TriggerServerEvent("lvc_TogPwrcallState_s", source12,state_pwrcall[veh])
								TriggerServerEvent("lvc_SetAirManuState_s", source12,state_airmanu[veh])
							end
