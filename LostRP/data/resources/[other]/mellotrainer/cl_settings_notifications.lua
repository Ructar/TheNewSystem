...
local selfDeathMessage = "~o~Vous êtes ~s~morts."
local deathSuicideMessage = "~o~Vous ~s~avez commis un suicide."
...
AddEventHandler( 'mellotrainer:playerJoined', function( ID )
	...
	if ( featurePlayerNotifications and id ~= PlayerId() ) then
		...
		drawNotification( "~g~<C>"..name.."</C> ~s~vient d'arriver en ville." )
	end
end )
...
AddEventHandler( 'mellotrainer:playerLeft', function( person )
	if ( featurePlayerNotifications ) then
		drawNotification( "~r~<C>" .. person.name .. "</C> ~s~vient de quitter la ville." )
	end
end )
...
function handlePlayerDeathMessage( pedID, currentPed )
	...
	local msg = "~o~<C>" .. name .. "</C> ~s~est dans le coma."
	...
	if ( IsPedAPlayer( entity ) ) then
		...
		if ( kname == name ) then
			msg = "~o~<C>" .. name .. "</C> ~s~vient de se suicider"
		elseif ( kname == GetPlayerName( me ) )then
			msg = "~o~<C>Vous avez</C> ~s~" .. killActionFromWeaponHash( weaponHash ) .. " ~o~<C>" .. name .. "</C>~s~."
		else
			msg = "~y~<C>" .. kname .. "</C> vient de ~s~" .. killActionFromWeaponHash( weaponHash ) .. " ~o~<C>" .. name .. "</C>~s~."
		end
	end
