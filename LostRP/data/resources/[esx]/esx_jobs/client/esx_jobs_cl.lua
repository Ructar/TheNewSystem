...
function spawncar(spawnPoint, vehicle)
  local SpawnPointVehicleBusy = false
  local coords = spawnPoint.Pos
  if not IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 7.0) then
	SpawnPointVehicleBusy = false
  else
    SpawnPointVehicleBusy = true
  --hintToDisplay = _U('spawn_point_busy')
    TriggerEvent('esx:showNotification', _U('spawn_point_busy'))
  end
  hintToDisplay = "no hint to display"
  hintIsShowed = false
  TriggerServerEvent('esx_jobs:caution', "take", cautionVehicleInCaseofDrop, spawnPoint, vehicle, SpawnPointVehicleBusy)
end
