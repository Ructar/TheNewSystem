	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			if data.current.value == 'vehicle_list' then
				...
				if Config.EnablePlayerManagement and PlayerData.job ~= nil and 
					(PlayerData.job.grade_name == 'boss' or PlayerData.job.grade_name == 'gerant') then
					table.insert(elements, {label = 'Dubsta6x6', value = 'dubsta3'})
					table.insert(elements, {label = 'Schafter LWB (Armored)', value = 'SCHAFTER6'})
				end
	    ...
	    if data.current.value == 'get_stock' then
			...
			if data.current.value == 'boss_actions' then
				TriggerEvent('esx_society:openBossMenu', 'disco', function(data, menu)
					menu.close()
				end)
			end
...
function OpenDiscoHarvestMenu()
	...
	if Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.grade_name ~= 'videur' then
		local elements = {
		...
		ESX.UI.Menu.Open(
			...
			function(data, menu)
				...
				if data.current.value == 'limonade' then
					menu.close()
					TriggerServerEvent('esx_discojob:startHarvest4')
				end

				if data.current.value == 'amidon' then
					menu.close()
				...
				if data.current.value == 'agrume' then
					menu.close()
					TriggerServerEvent('esx_discojob:startHarvest14')
				end

				if data.current.value == 'cafeine' then
					menu.close()
					TriggerServerEvent('esx_discojob:startHarvest15')
				end
		...
		ESX.UI.Menu.Open(
			...
			function(data, menu)
				...
				if data.current.value == 'schweppes' then
					menu.close()
					TriggerServerEvent('esx_discojob:startCraft14')
				end

				if data.current.value == 'cafe' then
					menu.close()
					TriggerServerEvent('esx_discojob:startCraft15')
				end
			end,
			function(data, menu)
				menu.close()
				CurrentAction     = 'disco_craft_menu'
				CurrentActionMsg  = 'Appuyez sur ~INPUT_CONTEXT~ pour préparer des boissons.'
				CurrentActionData = {}
			end
		)
	else
...
AddEventHandler('esx_discojob:hasEnteredMarker', function(zone)
	...
	if zone == 'Craft' then
		CurrentAction     = 'disco_craft_menu'
		CurrentActionMsg  = 'Appuyez sur ~INPUT_CONTEXT~ pour accéder au menu de préparation.'
		CurrentActionData = {}
	end
...
AddEventHandler('esx_discojob:hasExitedMarker', function(zone)
	...
	if zone == 'Craft' then
		...
		TriggerServerEvent('esx_discojob:stopCraft14')
		TriggerServerEvent('esx_discojob:stopCraft15')
	end
	...
	if zone == 'Garage' then
		...
		TriggerServerEvent('esx_discojob:stopHarvest14')
		TriggerServerEvent('esx_discojob:stopHarvest15')
	end
...
Citizen.CreateThread(function()		
	...
	AddTextComponentString("Récolte Téqui-la-la")
        ...
        if CurrentAction ~= nil then
            ...
            if IsControlJustReleased(0, 38) and PlayerData.job ~= nil and PlayerData.job.name == 'disco' then                
                ...
                if CurrentAction == 'delete_vehicle' then
                    ...
                    if hash == GetHashKey('stretch') or hash == GetHashKey('dubsta3') or hash == GetHashKey('SCHAFTER3') or hash == GetHashKey('SCHAFTER6') then
                        if Config.MaxInService ~= -1 then
                            TriggerServerEvent('esx_service:disableService', 'disco')
                        end                        
                        DeleteVehicle(vehicle)
                    else
                        ESX.ShowNotification('Vous ne pouvez ranger que des ~b~véhicules du Tequi-la-la~s~.')
                    end
                end
                if CurrentAction == 'remove_entity' then
